...
def _true_on_policy_mode(args: Namespace) -> bool:
    return bool(getattr(args, "true_on_policy_mode", False))


def _attn_implementation(args: Namespace) -> str:
    return getattr(args, "attn_implementation", "flash_attention_2")


def _unpack_rollout_engine_info(rollout_engine_info):
    if len(rollout_engine_info) == 5:
        rollout_engines, rollout_engine_lock, num_new_engines, _engine_gpu_counts, _engine_gpu_offsets = (
            rollout_engine_info
        )
        return rollout_engines, rollout_engine_lock, num_new_engines
    if len(rollout_engine_info) == 3:
        return rollout_engine_info
    raise ValueError(f"Unexpected rollout engine info shape: {len(rollout_engine_info)}")
...
class FSDPTrainRayActor(TrainRayActor):
    ...
    def init(
        ...
        with init_context():
            model = self.get_model_cls().from_pretrained(
                ...
                attn_implementation=_attn_implementation(self.args),
            )
            ...
            with timer(f"{store_prefix}log_probs"), torch.no_grad():
                for batch in self.prof.iterate_train_log_probs(
                    ...
                    log_probs_result, entropy_result = get_logprob_and_entropy(
                        ...
                        allow_compile=not _true_on_policy_mode(self.args),
    ...
    def _packed_data(
        ...
        packed_batches = []
        mbs_size_list = []
        global_batch_size = int(rollout_data.get("dynamic_global_batch_size", self.args.global_batch_size))
        local_batch_size = global_batch_size // self.dp_size
        assert (
            global_batch_size % self.dp_size == 0
        ), f"global_batch_size {global_batch_size} is not divisible by dp_world_size {self.dp_size}"
        ...
        if self.args.use_dynamic_batch_size:
            ...
            num_microbatches = [global_batch_size // (self.args.micro_batch_size * self.dp_size)] * (
    ...
    def train(self, rollout_id: int, rollout_data_ref: Box, external_data=None) -> None:
        ...
        Parameters:
            ...
            external_data: ActorGroup-compatible extension point used by other
                backends. FSDP actor training does not consume it currently.
        """
        if self.args.offload_train:
            self.wake_up()
        ...
        with inverse_timer("train_wait"), timer("train"):
            ...
            if self.args.debug_rollout_only:
                return
            if not self._has_global_trainable_tokens(rollout_data):
                if dist.get_rank() == 0:
                    logger.warning(
    ...
    def _has_global_trainable_tokens(self, rollout_data) -> bool:
        local_count = 0.0
        for loss_mask in rollout_data.get("loss_masks", []):
            if isinstance(loss_mask, torch.Tensor):
                local_count += float(loss_mask.sum().item())
            else:
                local_count += float(sum(loss_mask))
        count_tensor = torch.tensor([local_count], device=torch.cuda.current_device())
        dist.all_reduce(count_tensor, op=dist.ReduceOp.SUM, group=self.dp_group)
    ...
    def _log_rollout_data(self, rollout_id: int, rollout_data, packed_batches):
        ...
        for metric_key in ["log_probs", "rollout_log_probs", "ref_log_probs", "advantages", "returns"]:
            ...
            log_dict[f"rollout/{metric_key}"] = (val / global_batch_size).item()
        if dist.get_rank() == 0:
        ...
        if self.args.ci_test and _true_on_policy_mode(self.args):
    ...
    def _train_core(self, rollout_id: int, rollout_data) -> None:
        ...
        assert (
            len(grad_accum) > 0
        ), f"Invalid grad_accum {grad_accum} for micro_batch_size {self.args.micro_batch_size} and global_batch_size {global_batch_size}"
    ...
    def _train_step(self, packed_batch, reported_accum, mbs_id, grad_accum, global_batch_size):
    ...
    def update_weights(self) -> None:  # type: ignore[override]
        ...
        get_engines = getattr(self.rollout_manager, "get_updatable_engines_and_lock", None)
        if get_engines is None:
            get_engines = self.rollout_manager.get_rollout_engines_and_lock
        rollout_engines, rollout_engine_lock, num_new_engines = _unpack_rollout_engine_info(
            ray.get(get_engines.remote())
        )
        if num_new_engines > 0:
            self.weight_updater.connect_rollout_engines(rollout_engines, rollout_engine_lock)
            dist.barrier(group=get_gloo_group())
            if dist.get_rank() == 0:
                clear_num_new_engines = getattr(self.rollout_manager, "clear_updatable_num_new_engines", None)
                if clear_num_new_engines is None:
                    clear_num_new_engines = self.rollout_manager.clear_num_new_engines
                ray.get(clear_num_new_engines.remote())
