...
def load_lora_checkpoint(model: torch.nn.Module, path: Path) -> None:
    ...
    should_load = [bool(full_state) if dist.get_rank() == 0 else False]
    dist.broadcast_object_list(should_load, src=0)
    ...
    if should_load[0]:
        ...
        logger.warning(f"No matching LoRA parameters found in {adapter_file}; skipping load.")
