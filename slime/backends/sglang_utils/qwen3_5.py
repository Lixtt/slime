...
def maybe_prepare_qwen35_text_model(model_path: str, *, language_only: bool) -> str:
    ...
    hf_config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
    if getattr(hf_config, "model_type", None) not in {"qwen3_5", "qwen3_5_moe"} or not hasattr(
...
def _patch_transformers_qwen35_backend_compat() -> None:
    try:
    ...
    for cls in (Qwen3_5ForCausalLM, Qwen3_5ForConditionalGeneration):
        if getattr(cls, "_supports_attention_backend", None) is not True:
            cls._supports_attention_backend = True
            patched.append(cls.__name__)

    if patched:
        logger.info(
            "Enabled Transformers attention backend compatibility for Qwen3.5 classes: %s",
            ", ".join(patched),
        )
...
def patch_sglang_qwen35() -> None:
    ...
    _patch_transformers_qwen35_backend_compat()

    try:
        from transformers.models.qwen3_5.configuration_qwen3_5 import Qwen3_5TextConfig
        from transformers.models.qwen3_5_moe.configuration_qwen3_5_moe import Qwen3_5MoeTextConfig

        def _basic_layers_block_type(self):
            layer_types = getattr(self, "layer_types", None) or []
            if not layer_types:
                return ["attention"] * int(getattr(self, "num_hidden_layers", 0) or 0)
            return [
                "attention" if layer_type == "full_attention" else layer_type
                for layer_type in layer_types
            ]

        def _basic_linear_layer_ids(self):
            return [
                idx
        ...
        def _basic_full_attention_layer_ids(self):
            return [
                idx
                for idx, layer_type in enumerate(self.layers_block_type)
        ...
        for config_cls in (Qwen3_5TextConfig, Qwen3_5MoeTextConfig):
            config_cls.layers_block_type = property(_basic_layers_block_type)
            config_cls.linear_layer_ids = property(_basic_linear_layer_ids)
            config_cls.full_attention_layer_ids = property(_basic_full_attention_layer_ids)
        if not hasattr(Qwen3_5MoeTextConfig, "norm_topk_prob"):
            Qwen3_5MoeTextConfig.norm_topk_prob = True
    except Exception as exc:
        logger.info("Could not install Qwen3.5 config compatibility properties: %s", exc)
    ...
    try:
        ...
        logger.info(
            "Native SGLang Qwen3.5 patch unavailable; using Transformers fallback instead: %s",
            exc,
        )
        return

    original_qwen35_dense_cls = qwen3_5_model.Qwen3_5ForCausalLM
    original_qwen35_moe_cls = qwen3_5_model.Qwen3_5MoeForCausalLM
    ...
    class PatchedQwen35MoeForCausalLM(nn.Module):
        def __init__(
            self,
            ...
            self.config = config
            self.quant_config = quant_config
            self.model = original_qwen35_moe_cls(
                config=config,
                quant_config=quant_config,
                prefix=add_prefix("model", prefix),
            )
        ...
        def set_dflash_layers_to_capture(self, layers_to_capture: list[int]):
            return self.model.set_dflash_layers_to_capture(layers_to_capture)
        ...
        def load_weights(self, weights):
            return original_qwen35_moe_cls.load_weights(self, weights)
    ...
    PatchedQwen35MoeForCausalLM.__name__ = "Qwen3_5MoeForCausalLM"
    qwen3_5_model.Qwen3_5MoeForCausalLM = PatchedQwen35MoeForCausalLM
    ...
    for config_cls in (Qwen3_5TextConfig, Qwen3_5MoeTextConfig):
        config_cls.layers_block_type = property(_layers_block_type)
        config_cls.linear_layer_ids = property(_linear_layer_ids)
        config_cls.full_attention_layer_ids = property(_full_attention_layer_ids)
        config_cls.mamba2_cache_params = property(_mamba2_cache_params)
    if not hasattr(Qwen3_5MoeTextConfig, "norm_topk_prob"):
        Qwen3_5MoeTextConfig.norm_topk_prob = True
    ...
    def _hybrid_gdn_config(self):
        ...
        if isinstance(config, (Qwen3_5Config, Qwen3_5TextConfig, Qwen3_5MoeConfig, Qwen3_5MoeTextConfig)):
            _ensure_qwen35_attention_layer_ids(config)
            return config
        if isinstance(config, Qwen3_5VisionConfig):
            text_config = getattr(config, "text_config", None)
            if isinstance(text_config, (Qwen3_5Config, Qwen3_5TextConfig, Qwen3_5MoeConfig, Qwen3_5MoeTextConfig)):
...
def _get_shadow_model_dir(model_path: str) -> Path:
    ...
    return base_dir / f"qwen3_5_text_v11_{source_hash}"
...
def _populate_shadow_model_dir(source_dir: Path, target_dir: Path, hf_config) -> None:
    ...
    if quantization_config is not None and "quantization_config" not in config_dict:
        ...
        if modules_to_not_convert:
            quantization_config["modules_to_not_convert"] = [
                layer.replace("model.language_model.", "model.")
                for layer in modules_to_not_convert
            ]
        packed_modules_mapping = dict(quantization_config.get("packed_modules_mapping") or {})
        packed_modules_mapping.setdefault("in_proj_ba", ["in_proj_b", "in_proj_a"])
        packed_modules_mapping.setdefault("in_proj_qkvz", ["in_proj_qkv", "in_proj_z"])
        quantization_config["packed_modules_mapping"] = packed_modules_mapping
        config_dict["quantization_config"] = quantization_config
    if is_moe:
        config_dict.setdefault("norm_topk_prob", True)
