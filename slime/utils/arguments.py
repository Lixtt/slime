    ...
    def add_slime_arguments(parser):
        ...
        def add_fault_tolerance_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=float,
                default=30.0,
                help="Interval in seconds between rollout engine health endpoint checks during generate/eval.",
            )
            parser.add_argument(
                "--rollout-health-check-timeout",
                type=float,
                default=30.0,
                help="Timeout in seconds to wait for a rollout engine health endpoint response before killing it.",
            )
            parser.add_argument(
        ...
        def add_rollout_buffer_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=str,
                default="qwen",
                choices=["qwen", "qwen3", "qwen3_5", "glm", "gemma4", "distill_qwen"],
                help="Loss mask type",
            )
            parser.add_argument(
...
def slime_validate_args(args):
    ...
    trainable_only_load_requested = bool(
        getattr(args, "megatron_trainable_only_load", False)
