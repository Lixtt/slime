...
class Qwen35BridgeAttention(MegatronModule):
    ...
    def hf_forward(self, hidden_states: torch.Tensor, packed_seq_params: PackedSeqParams | None) -> torch.Tensor:
        ...
        *,
        ...
        if mpu.get_context_parallel_world_size() > 1:
            if packed_seq_params is None:
                raise ValueError("Qwen3.5 context parallel attention requires packed_seq_params")
    ...
    def forward(
        ...
        *,
        ...
        if position_ids is None:
            ...
            elif decoder_input is not None:
                seq_len, batch_size = decoder_input.shape[0], decoder_input.shape[1]
                position_ids = torch.arange(seq_len, dtype=torch.long, device=decoder_input.device).unsqueeze(0)
                position_ids = position_ids.expand(batch_size, -1)
...
def _get_qwen35_layer_spec(provider: "Qwen35VLModelProvider", vp_stage=None):
    config = provider
    if not (getattr(config, "num_experts", None) or getattr(config, "num_moe_experts", None)):
        config.moe_layer_freq = [0] * config.num_layers

    kwargs = {"use_transformer_engine": bool(getattr(provider, "use_transformer_engine", True))}
